...
export default function OnboardingPage() {
    ...
    const [serviceStatus, setServiceStatus] = useState<{ isServiceable: boolean | null, message: string }>({
        isServiceable: null,
        message: ""
    });

    const { getPosition, stopTracking, loading: geoLoading, error: geoError } = useGeolocation();
    ...
    const checkServiceability = async (lat: number, lng: number) => {
        try {
            const res = await api.get<{ serviceable: boolean, message?: string, zone?: any }>(`/check-serviceability?lat=${lat}&lng=${lng}`);
            setServiceStatus({
                isServiceable: res.serviceable,
                message: res.serviceable ? `Great! We serve your area (${res.zone.name}).` : (res.message || 'We do not currently serve this location.')
            });
            if (res.serviceable && res.zone?.city_id) {
                // Optionally update city based on zone
            }
        } catch (err) {
            console.error(err);
        }
    ...
    const handleGeoSuccess = useCallback((newCoords: { latitude: number; longitude: number }, accuracy: number) => {
        setFormData(prev => ({
            ...prev,
            address: {
                ...prev.address,
        ...
        handleReverseGeocode(newCoords.latitude, newCoords.longitude);
        checkServiceability(newCoords.latitude, newCoords.longitude);
    }, []);
                        ...
                        {step === 1 && (
                            ...
                            >
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label htmlFor="onboarding-full-name" className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Full Name</label>
                                    ...
                                    <div>
                                        <label htmlFor="onboarding-email" className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Email Address</label>
                                        <div className="relative">
                                    ...
                                    <div>
                                        <div className="flex justify-between items-center mb-1.5 ml-1">
                                            <label htmlFor="onboarding-whatsapp" className="block text-xs font-black text-slate-400 uppercase tracking-widest">WhatsApp Number</label>
                                            <button
                        ...
                        {step === 3 && (
                            ...
                            >
                                ...
                                <div className="space-y-4">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => getPosition({ enableHighAccuracy: true, timeout: 15000 }, true, handleGeoSuccess)}
                                        disabled={geoLoading}
                                        className={cn(
                                ...
                                <div>
                                    <label htmlFor="onboarding-street" className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Street / Landmark / Gate</label>
                                    <input
                                    ...
                                    {serviceStatus.isServiceable !== null && (
                                        <div className={cn(
                                            "mt-4 p-3 rounded-xl border text-sm font-bold flex items-center gap-2",
                                            serviceStatus.isServiceable
                                                ? "bg-green-50 text-green-700 border-green-200"
                                                : "bg-red-50 text-red-700 border-red-200"
                                        )}>
                                            {serviceStatus.isServiceable ? <CheckCircle2 size={16} /> : "⚠️"}
                                ...
                                <div className="flex gap-4 pt-4">
                                    ...
                                    <button
                                        ...
                                        disabled={loading || !formData.address.street || serviceStatus.isServiceable === false}
                                        className="flex-1 h-14 bg-brand-green text-white rounded-2xl font-bold text-lg shadow-lg shadow-brand-green/20 flex items-center justify-center gap-2 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
